...
encoder's output with the other decoder, and check the packed
encoder against the spec's worked packing example.
...
- ``test_packed_vector`` -- run ``capn_deflate`` on the two-word
  packing example from the `encoding spec <https://capnproto.org/encoding.html#packing>`_
  and assert the output is ``51 08 03 02 31 19 aa 01``.
