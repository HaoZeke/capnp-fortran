...
* c-capnproto golden master

This tier builds the same Cap'n Proto message two ways -- with the
reference C encoder [[https://github.com/opensourcerouting/c-capnproto][c-capnproto]]
...
- =test_packed_vector= -- run =capn_deflate= on the two-word
  packing example from the [[https://capnproto.org/encoding.html#packing][encoding spec]]
