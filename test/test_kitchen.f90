...
module test_kitchen
   ...
   private
   public :: collect_kitchen

contains

   subroutine collect_kitchen(testsuite)
      type(unittest_type), allocatable, intent(out) :: testsuite(:)
      testsuite = [ &
           new_unittest("consts", t_consts), &
           new_unittest("defaults-on-fresh-struct", t_defaults_on_fresh_struct), &
           new_unittest("values-roundtrip", t_values_roundtrip)]
   end subroutine collect_kitchen
   ...
   subroutine t_consts(error)
      type(error_type), allocatable, intent(out) :: error
      type(capnp_ptr_t) :: l
      type(vec3_t) :: v
      integer :: err
      call check_(error, ANSWER == 42_int64, 'const: answer')
      call check_(error, abs(TAU - 6.283185307179586_real64) < 1.0e-15_real64, 'const: tau')
      call check_(error, GREETING == 'hey there', 'const: greeting')
      call check_(error, STATUS_BUSY_E == 1, 'const: imported enum')
      call check_(error, size(MAGIC) == 4 .and. MAGIC(0) == -54_int8 .and. &
                  MAGIC(3) == 13_int8, 'const: data bytes')
      l = primes(err)
      call check_(error, err == CAPNP_OK .and. capnp_list_len(l) == 4_int64, 'const: list len')
      call check_(error, capnp_list_get_i32(l, 0_int64, err) == 2_int32 .and. &
                  capnp_list_get_i32(l, 3_int64, err) == 7_int32, 'const: list values')
      v = home(err)
      call check_(error, err == CAPNP_OK, 'const: struct resolves')
      call check_(error, vec3_x_get(v) == -1.0_real64 .and. vec3_y_get(v) == 0.5_real64 &
   ...
   subroutine t_defaults_on_fresh_struct(error)
      type(error_type), allocatable, intent(out) :: error
      ...
      call check_(error, err == CAPNP_OK, 'defaults: new root')

      call check_(error, sink_flag_get(s), 'defaults: bool true')
      call check_(error, sink_count_get(s) == -7_int32, 'defaults: int -7')
      call check_(error, sink_ratio_get(s) == 2.5_real64, 'defaults: float 2.5')
      call sink_label_get(s, str, err)
      call check_(error, str == 'unnamed', 'defaults: text')
      call sink_payload_get(s, b, err)
      call check_(error, size(b) == 4 .and. b(lbound(b, 1)) == -34_int8, 'defaults: data blob')
      call check_(error, sink_state_get(s) == STATUS_BUSY_E, 'defaults: imported enum field')

      v = sink_origin_get(s, err)
      call check_(error, err == CAPNP_OK, 'defaults: struct blob resolves')
      call check_(error, vec3_x_get(v) == 1.5_real64 .and. vec3_y_get(v) == 2.5_real64 &
      ...
      block
         ...
         call check_(error, err == CAPNP_OK .and. capnp_list_len(l) == 3_int64, 'defaults: list blob len')
         call check_(error, capnp_list_get_i64(l, 0_int64, err) == 3_int64 .and. &
   ...
   subroutine t_values_roundtrip(error)
      type(error_type), allocatable, intent(out) :: error
      ...
      call check_(error, err == CAPNP_OK, 'values: built')

      call capnp_serialize_bytes(msg, bytes, err)
      call capnp_deserialize_bytes(bytes, rmsg, err)
      s = sink_read_root(rmsg, err)

      call check_(error, .not. sink_flag_get(s), 'values: bool')
      call check_(error, sink_count_get(s) == 123456_int32, 'values: int')
      call check_(error, sink_ratio_get(s) == 0.25_real64, 'values: float')
      call sink_label_get(s, str, err)
      call check_(error, str == 'named after all', 'values: text')
      call check_(error, sink_state_get(s) == STATUS_DOWN_E, 'values: enum')
      v = sink_origin_get(s, err)
      call check_(error, vec3_x_get(v) == 9.0_real64 .and. vec3_z_get(v) == 7.0_real64, &
                  'values: nested struct')
      call sink_tags_get_elem(s, 1, str, err)
      call check_(error, str == 'blue', 'values: List(Text) elem')
      l = sink_grid_get(s, err)
      row = capnp_getp(l, 1, err)
      call check_(error, capnp_list_get_i32(row, 1_int64, err) == 12_int32, 'values: List(List)')
      l = sink_spots_get(s, err)
      v%p = capnp_list_get_struct(l, 0, err)
      call check_(error, vec3_x_get(v) == 0.5_real64, 'values: List(struct)')
      any = sink_stuff_get(s, err)
      call check_(error, any%kind == CAPNP_PK_LIST .and. capnp_list_len(any) == 3_int64, &
                  'values: anyPointer')
      call capnp_message_free(msg)
      call capnp_message_free(rmsg)
   end subroutine t_values_roundtrip

end module test_kitchen
